...
from modules.video_frame_extract import (
    TimeSegment, VIDEO_EXTENSIONS, probe_video_with_ffmpeg, 
    is_video, extract_numbers, get_file_size,
    extract_frames_opencv, extract_frames_ffmpeg, _get_ffmpeg_path
)
from modules.video_concat import run_ffmpeg_concat
from modules.file_cleanup import collect_orphan_jpg, clean_orphan_files
from modules.file_rename import rename_files_by_number
...
class MedicalFileProcessorApp:
    ...
    def create_rename_tab(self) -> tk.Frame:
        ...
        preview_frame.pack(fill=tk.BOTH, expand=True, pady=(8, 8), padx=16)
        tk.Label(
            preview_frame,
            text="重命名预览（原名 → 新名）",
        ...
        # 左侧：表格
        left_frame = tk.Frame(content_container, bg=COLORS['bg_card'])
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 20))
        
        # 使用Treeview实现表格
        columns = ('seq', 'old_name', 'arrow', 'new_name')
        self.rename_tree = ttk.Treeview(left_frame, columns=columns, show='headings', height=18)
        ...
        # 右侧：控制选项（横向排列）- 固定宽度避免影响左侧表格
        right_frame = tk.Frame(content_container, bg=COLORS['bg_card'], width=400)
        ...
        action_frame.pack(fill=tk.X, side=tk.BOTTOM, pady=(0, 12), padx=16)
        tk.Label(
            action_frame,
            text="操作",
            font=("Microsoft YaHei UI", 12, "bold"),
    ...
    def _show_preview_image(self, out_path, token):
        ...
        if token != getattr(self, '_preview_token', None):
            ...
            return
        try:
            img_tk = tk.PhotoImage(file=out_path)
            img_w = img_tk.width()
            img_h = img_tk.height()
            canvas_w, canvas_h = 640, 320
            if img_w > canvas_w or img_h > canvas_h:
                ...
                new_w = max(1, int(img_w * ratio))
                new_h = max(1, int(img_h * ratio))
                img_tk = img_tk.subsample(max(1, img_w // new_w), max(1, img_h // new_h))
            self._frame_img_cache = img_tk
            self.canvas_frame.delete("all")
            x = (canvas_w - img_tk.width()) // 2
            y = (canvas_h - img_tk.height()) // 2
